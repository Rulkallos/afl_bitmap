...
void save_bitmap(afl_state_t *afl, struct queue_entry *q){
  u8 *seed_name = strrchr(q->fname, '/');
  q->bitmap_name = alloc_printf(
        "%s/bitmap/%s", afl->out_dir, seed_name);
  s32 fd = permissive_create(afl, q->bitmap_name);

  if (likely(fd >= 0)) {
    ck_write(fd, afl->fsrv.trace_bits, afl->fsrv.map_size, q->bitmap_name);
    close(fd);
  }
}
#endif
